...
from .models import Product, Customer, Sale, SaleProduct, Article, FAQ, Employee, Review, PromoCode, Category, Vacancy, \
    Partner, Cart, CartProduct, Company
...
def product_detail(request, pk):
    """Детальная страница товара с кнопкой добавления в корзину"""
...
def cart_view(request):
    ...
    total_price = sum(item.get_total_price() for item in cart_products)
    ...
    if request.method == 'POST':
        ...
        if cart_product_id and quantity:
            try:
...
def checkout(request):
    ...
    if request.method == 'POST':
        city = request.POST.get('city', 'Минск')
        ...
        try:
            ...
            for cart_product in cart_products:
                item_total = cart_product.product.price * cart_product.quantity
            ...
            messages.success(request, f'Заказ №{sale.id} оформлен! Сумма: {total_price} BYN')
            return redirect('cart_view')
...
def home(request):
    ...
    return render(request, 'home.html', {'article': latest_article_obj, 'user': request.user, 'partners': partners})


def about_us(request):
    company = Company.objects.first()
    return render(request, "about_us.html", {"company": company})
